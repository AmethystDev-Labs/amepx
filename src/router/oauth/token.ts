...
import * as token from "../../models/router/oauth/token.js";
...
async function issueTokens(params: {
  ...
  const accessTokenTtlSeconds = oauth.getOAuthAccessTokenTtlSeconds();
  const refreshTokenTtlSeconds = oauth.getOAuthRefreshTokenTtlSeconds();

  const accessToken = oauth.generateOpaqueToken(32);
  const refreshToken = oauth.generateOpaqueToken(48);
  ...
  await OAuthTokenModel.create({
    ...
    accessTokenHash: oauth.hashTokenValue(accessToken),
    refreshTokenHash: oauth.hashTokenValue(refreshToken),
    accessExpiresAt,
    refreshExpiresAt,
  ...
  const response: {
    ...
    scope: oauth.serializeScope(params.scope),
  };
...
function parseTokenBody(rawBody: unknown): {
  ...
  if (typeof rawBody === "string") {
    ...
    return parsed as unknown as NewType;
  }
...
export const oauthTokenRouter = new Elysia({ prefix: "/oauth" }).post(
  ...
  async ({ body, headers, set, request }) => {
    ...
    const authorizationHeader =
      typeof headers.authorization === "string"
        ...
        : undefined;
    const parsedBasic =
      oauth.parseBasicAuthorizationHeader(authorizationHeader);
    ...
    if (!oauth.verifyClientSecret(client.clientSecretHash, clientSecret)) {
      return oauthErrorResponse(
        set,
        401,
        "invalid_client",
    ...
    if (input.grant_type === "refresh_token") {
      ...
      const refreshTokenHash = oauth.hashTokenValue(input.refresh_token);
  ...
  {
    body: token.oauthTokenBodySchema,
    response: {
      200: token.oauthTokenSuccessSchema,
      400: token.oauthTokenErrorSchema,
      401: token.oauthTokenErrorSchema,
    },
  },
);
