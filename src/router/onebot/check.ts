...
export const onebotCheckRouter = new Elysia({ prefix: "/onebot" }).post(
    ...
    async ({ body, set }) => {
        ...
        let resolved: Awaited<ReturnType<typeof oneBotService.resolveCodeFromGroup>>;
        try {
        ...
        if (!resolved.matchedMessage) {
            return {
                ...
                request_id: authorizationRequest.requestId,
                code: authorizationRequest.code,
                message: "Code not found in recent group history. Send the code to group and retry.",
            };
        }
        ...
        authorizationRequest.user = {
            ...
            nickname: resolved.memberInfo?.nickname || resolved.matchedMessage.sender?.nickname,
